...
{ 
  ...
  // Set the beam distribution and momentum spreading from twiss parameters
  // the passed values should override the /gun/direction, vertex, vertexsigma, tilt, tiltsigma, ... everything !
  setUseBeamFromTwissCmd = new G4UIcmdWithABool("/gun/twiss/use",this);
  setUseBeamFromTwissCmd->SetGuidance("Set the beam distribution and momentum spreading from twiss parameters");
  setUseBeamFromTwissCmd->SetGuidance("pass this after you set the beam momentum or energy.");
  // setUseBeamFromTwissCmd->SetGuidance("So if you reset momentum in GUI, you should also run this command");
  setUseBeamFromTwissCmd->SetGuidance("this should override /gun/direction, vertex, vertexsigma, tilt, tiltsigma, ... everything !");
  setUseBeamFromTwissCmd->SetGuidance("dispersion ? polarization ?");
  setUseBeamFromTwissCmd->SetParameterName("UseTwiss",true,true);

  setBeamXFromTwissCmd = new G4UIcmdWith3Vector("/gun/twiss/x",this);
  setBeamXFromTwissCmd->SetGuidance("normalized emittance [mm mrad], beta [m], alpha [dimensionless] for X direction");
  setBeamXFromTwissCmd->SetParameterName("epsNX","betaX", "alphaX",true,true);

  setBeamYFromTwissCmd = new G4UIcmdWith3Vector("/gun/twiss/y",this);
  setBeamYFromTwissCmd->SetGuidance("normalized emittance [mm mrad], beta [m], alpha [dimensionless] for Y direction");
  setBeamYFromTwissCmd->SetParameterName("epsNY","betaY", "alphaY",true,true);
...
musrPrimaryGeneratorMessenger::~musrPrimaryGeneratorMessenger()
{
  ...
  delete setUseBeamFromTwissCmd;
  delete setBeamXFromTwissCmd;
  delete setBeamYFromTwissCmd;
  delete setvertexCmd;
...
{ 
  ...
  if( command == setUseBeamFromTwissCmd)
  { G4bool UsingTwiss = setUseBeamFromTwissCmd->GetNewBoolValue(newValue);
    musrAction->SetUseTwiss(UsingTwiss);
    G4cout<< "<debugging> PGA Messenger use twiss: " << UsingTwiss << G4endl;
    if(UsingTwiss){musrAction->setupCovariance();};}
  if( command == setBeamXFromTwissCmd)
  { musrAction->SetBeamFromTwiss(setBeamXFromTwissCmd->GetNew3VectorValue(newValue),"x"); }
  if( command == setBeamYFromTwissCmd)
  { musrAction->SetBeamFromTwiss(setBeamYFromTwissCmd->GetNew3VectorValue(newValue),"y"); }
